...
const PlayerContent: React.FC<PlayerContentProps> = ({ song, songUrl }) => {
  ...
  const onPlayNext = () => {
    if (player.ids.length === 0) return; //check if there is a active songs

    const currentIndex = player.ids.findIndex((id) => id === player.activeId);
    const nextSong = player.ids[currentIndex + 1];

    if (!nextSong) return player.setId(player.ids[0]); //reset/return to 1st song if there is no next song
  ...
  const onPlayPrevious = () => {
    ...
    if (!prevSong) return player.setId(player.ids[player.ids.length - 1]); //reset/return to 1st song if there is no prev song
